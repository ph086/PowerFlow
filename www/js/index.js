...
if (decisao==true){
	...
	tx.executeSql('delete from linhas');
	tx.executeSql('delete from barras');
	tx.executeSql('SELECT * FROM linhas', [], barras_refresh, errorDB);
	
	function barras_refresh(){
	tx.executeSql('SELECT * FROM barras', [], data_view, errorDB);
	}
	...
	{   
		...
		if((rs&&xs&&bsh&&origem&&destino !=="") && (origem!==destino)){
	
		tx.executeSql('INSERT INTO linhas (nome, rs, xs, bsh, origem, destino) VALUES ("'+ nome +'", "'+ rs +'", "'+ xs +'", "'+ bsh +'", "'+ origem +'", "'+ destino +'")');
		window.location = 'lista de componentes.html';
		}
		else if((origem==destino)&&(origem !=="")){
			alert("Preencha os campos com valores distintos de barras!")
...
function processo_de_iteracao(){
 ...
 var matriz_de_equacoes = equacoes.subset(math.index(y,0));
 

for(var x=0;x<numerodeequacoes;x++){	
var equacao_simplifica = math.simplify(''+matriz_de_equacoes+'', {[''+todasincognitas.get([x,0])+'']: valor_das_incognitas.get([x,0]) })
  matriz_de_equacoes = equacao_simplifica;
}

deltaPQ.subset(math.index(y,0),math.subtract(Valores_de_entrada.get([y,0]),math.eval(''+matriz_de_equacoes+'')))
...
for(var a=0;a<qtde_barras;a++){
	     for(var b=0;b<qtde_barras;b++){
			...
			var td2 = document.createElement("td");
				 td2.appendChild(document.createTextNode(math.round(math.multiply(potenciaativa.get([a,b]),localStorage.getItem("Sbase")),3)));
				 tr.appendChild(td2);
			var td3 = document.createElement("td");
				 td3.appendChild(document.createTextNode(math.round(math.multiply(potenciareativa.get([a,b]),localStorage.getItem("Sbase")),3)));
				 tr.appendChild(td3);
			var td4 = document.createElement("td");
				 td4.appendChild(document.createTextNode(math.round(math.multiply(perdaativa.get([a,b]),localStorage.getItem("Sbase")),3)));
				 tr.appendChild(td4);
			var td5 = document.createElement("td");
				 td5.appendChild(document.createTextNode(math.round(math.multiply(perdareativa.get([a,b]),localStorage.getItem("Sbase")),3)));
				 tr.appendChild(td5);
		...
		document.getElementById("resultados").appendChild(tabela);
				...
				//CABEÇALHOS DA TABELA
		var titulos = ["BARRA", "TENSÃO", "ÂNGULO"];
		var subtítulos = ["PU", "RAD"];
		var tabela = document.createElement("table");
		var cabecalho = document.createElement("thead");
		var corpo = document.createElement("tbody");
		tabela.appendChild(cabecalho);
...
for(var a=0;a<qtde_barras;a++){
			...
			corpo.appendChild(tr);
			 ...
			 var td1 = document.createElement("td");
				 td1.appendChild(document.createTextNode(math.add(a,1)));
				 tr.appendChild(td1);
				 
			var td2 = document.createElement("td");
				 td2.appendChild(document.createTextNode(math.round(valoresfinaistensao.get([a,0]),3)));
				 tr.appendChild(td2);
			var td3 = document.createElement("td");
				 td3.appendChild(document.createTextNode(math.round(valoresfinaistheta.get([a,0]),3)));
				 tr.appendChild(td3);
		...
		document.getElementById("resultados2").appendChild(tabela);
		document.getElementById("fluxos_linhas").style.display = "inline";
}
